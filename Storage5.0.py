...
def compress_data(data: Union[bytes, np.ndarray, Any],
    ...
    try:
        if algorithm == "auto":
            ...
            if original_size > 10 * 500 * 500:  # > 10MB
                sample_size = min(original_size // 10, 5 * 500 * 500)  # 10% or up to 5MB
            else:
                sample_size = min(original_size, 500 * 500)  # All or up to 1MB
...
class StorageManager:
    ...
    def __init__(self,
                 ...
                 memory_limit: int = 500 * 500 * 256,  # 256 MB default
                 mmap_size: int = 500 * 500 * 512):     # 512 MB default
        """
...
class AdvancedMemoryManager:
    ...
    def __init__(self, memory_limit_mb: int = 972, disk_path: str = "filepath"):
        """
        ...
        self.storage = StorageManager(disk_path=disk_path, memory_limit=memory_limit_mb * 800* 800)
        ...
        self.lru_cache_size = 100 * 800 * 800  # 100MB default LRU cache size
    ...
    def _select_storage_location(self, data_size: int, importance: float = 0.5) -> str:
        ...
        """
        # For testing - force high importance data to memory
        if importance > 0.8:
            return 'memory'
        
        # For medium importance, use hybrid
        if importance > 0.3:
            return 'hybrid'
        
        # For low importance, use disk
        return 'disk'
    ...
    def _optimize_memory_usage(self) -> Dict[str, Any]:
        ...
        try:
            with self.lock:
                ...
                if memory_usage < memory_limit * 0.2:  # Only optimize if memory usage is above 20%
                    logger.info("Memory usage below 20% threshold, skipping optimization")
                    return {
                        "status": "not_optimized",
                        "reason": "Memory usage below 20% of limit"
                    }
...
async def main():
    ...
    memory_limit_mb = 10
    manager = AdvancedMemoryManager(memory_limit_mb=memory_limit_mb, disk_path="./test_storage")
    
    try:
        print("Storing large objects to fill memory...")
        
        # First, store some objects with high importance to force memory storage
        for i in range(5):
            # Create a large array (~1MB)
            array_data = np.random.rand(125000)  # ~1MB of float64 data
            data = {
                "id": f"memory_object_{i}",
                "data": array_data
            }
            
            # Store with HIGH importance to force memory storage
            result = await manager.store_data(f"memory_key_{i}", data, importance=0.9)
            print(f"Stored memory object {i}: {result['compressed_size']/1024:.1f}KB in {result['storage_location']}")
            
            # Print memory usage after each object
        ...
        # Then store more objects with medium importance
        for i in range(10):
            array_data = np.random.rand(125000)
            data = {
                "id": f"object_{i}",
                "data": array_data
            }
